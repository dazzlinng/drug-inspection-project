...
# 读取药品列表  
def read_drugs():  
    file_path = r"d:\Mysystem\新建文件夹 (3)\摘要\extracted_drugs.txt"  
...
def save_groups(groups):  
    output_path = r"d:\Mysystem\新建文件夹 (3)\摘要\drug_groups.json"  
    with open(output_path, 'w', encoding='utf-8') as f:  
        json.dump(groups, f, ensure_ascii=False, indent=2)  
    print(f"分组信息已保存到 {output_path}") 
def main():  
    # 读取药品  
    drugs = read_drugs()  
    ...
    print(f"药品已分为 {len(groups)} 组，每组最多5个药品") 
    # 保存分组  
    save_groups(groups)  
    # 显示前几组作为示例  
    print("\n前5组药品:")  
    for i in range(min(5, len(groups))):  
        print(f"组 {i+1}: {', '.join(groups[i])}") 
